# -*- coding: utf-8 -*-
"""
Created on Tue May 11 20:48:40 2021
...
class Node(object):
    ...
    def genEvent(self):
        ...
        global no_high_events, no_med_events, no_low_events, high_TTL, med_TTL, low_TTL
            ...
            if self.rE > baseE:
                ...
                if T % PT == (self.ti + 4) % PT:
                    ...
                    if True :
                        ...
                        #sort neighbour node according to their nmc, activeness and remaining capacity
                        NMC_order = sorted(sorted(self.NMC.items(), key = lambda y : y[1][2], reverse = True), key = lambda z : z[1][0], reverse = True)
                    
                        # Send data to next gop or base station
                        if len(self.recBuf) > 0 :
                       
                            # case 1
                            # if BS is within the sensing range, 
                            # send all the data directly to the base station
                            
                            
                            #case 2 
                            #if base station not within the range
                            #then data will be send to neighbour towards base station
                            #if neighbour not present
                            #if no suitable neighbour founds, data will be dropped
                            
                            # have to change for mobile and fog nodes range
                            # currently same range for both fog and mobile
                            ...
                            if "E-" in self.ID : 
                                ...
                                elif len(NMC_order) > 0 :
                                    
                                    #sorted NMC , remaining capacity ,activeness, distance from base station
                                    #NMC_order = (node, (NMC, remaining capacity, activeness, distance))
                                    
                                    len_nmc = len(NMC_order)
                                    
                                    #index of nmc node
                                    i = 0
                                
                                    # own distance from base station
                                    # to compare with the neighbour, who is nearest to the base station
                                    self_distance = find_dist(self.my_coor[0],self.my_coor[1],BC[0],BC[1])
            
                                    #until has data to send, and neighbour to be send
                                    while len(self.recBuf)>0 and len_nmc>0 :
                                        
                                        # data send to high motif neighbour
                                        # data send towards base station
                                        if (NMC_order[i][1][3] < self_distance) :
                                            self.next_hop = NMC_order[i][0]
                                            j = 0
                                            
                                            while len(self.recBuf)>0 and j<NMC_order[i][1][1] :
                                                   
                                                item = self.recBuf.pop()
                                                    
                                                entities[self.next_hop].recBuf.append(item)
                                                self.rE -= fg_fg_E
                                                j += 1
                                                    
                                            entities[self.next_hop].recBuf = entities[self.next_hop].recBuf[-recBufferCapacity_fog:]
                                        i +=1
                                        len_nmc -=1 
                                ...
                                # BS within range
                                if find_dist(self.my_coor[0], self.my_coor[1], BC[0], BC[1]) <= sensing_range_mobile:
    ...
    def find_global_graph(self):
        ...
        global sensing_range, sensing_range_mobile, eG, mG, baseE
        ...
        while True:
            ...
            if T%10 == 1 :
                ...
                for  i  in range(eG+mG) :
                    if  entities[i].rE > 10:
                        alive += 1
                    tot_buffer += len(entities[i].recBuf)
                total_buffer_consumed.append(tot_buffer)
                no_alive.append(alive)
